...
export function OrchardMapCanvas({
  ...
  const asGeoJSONLayer = (layer: L.Layer): L.Layer & { toGeoJSON: () => GeoJSON.Feature } =>
      ...
      <MapContainer center={[34.1, 74.8]} zoom={14} className="h-full w-full">
        ...
        <FeatureGroup>
          <EditControl
            ...
            onCreated={event => {
              const geojson = asGeoJSONLayer(event.layer as L.Layer).toGeoJSON();
            ...
            onEdited={event => {
              event.layers.eachLayer(layer => {
                const geojson = asGeoJSONLayer(layer as L.Layer).toGeoJSON();
                const polygon = extractPolygon(geojson);
                if (!polygon) return;
