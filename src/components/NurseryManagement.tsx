  ...
  const handleMortalitySubmit = async (e: FormEvent) => {
    ...
    if (mortalityForm.replacementCost && parseFloat(mortalityForm.replacementCost) > 0) {
      await addFinanceEntry({
        ...
        date: mortalityForm.replacementDate || mortalityForm.deathDate,
        field_id: mortalityForm.fieldId || null,
        notes: `Cause: ${mortalityForm.causeOfDeath}`,
      });
    }
                    ...
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <div className="flex items-center gap-2">
                        <span className={`text-xs px-2 py-1 rounded-full font-semibold ${
                          batch.survival_rate >= 90 ? 'bg-green-100 text-green-700' :
                          batch.survival_rate >= 75 ? 'bg-yellow-100 text-yellow-700' :
                          'bg-red-100 text-red-700'
                        }`}>
                          {batch.survival_rate}% survival
                        </span>
